...
# 🖥️ Générer et 🚀 Déployer Allure Report dans github actions 
...
# 🛠️ ⚙️ Configuration de Playwright avec Xray (Jira)
# Prérequis
  ...
  7. Créer un fichier **ray-reporter.ts** et faire les config (Voir le fichier)
  ...
  # 🔔 Difference entre les 2 fichiers <xray-reporter.ts> et <xray-reporter-with-steps.ts>
   1. **xray-reporter.ts** la conf pour recupérer le résultat des cas de tests déjà existant sur Xray et Playwright, mapper les cas de test à l'aide des Tags (@Kan-1) et mettre le résultat dans une nouvelle Execution Test sur Xray.

   2. **xray-reporter-with-steps.ts** la conf pour créer de nouveaux cas de test direct sur Xray et les reliés à une nouvelle Execution Test avec leur résultat.
